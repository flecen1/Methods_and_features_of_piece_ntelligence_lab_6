# -*- coding: utf-8 -*-
"""
Генератор методичних вказівок для лабораторної роботи №6
"""
...
def add_list_item(doc, text, level=0):
    """Додає пункт списку"""
    para = doc.add_paragraph(text, style=f'List Bullet')
    for run in para.runs:
        run.font.size = Pt(12)
        run.font.name = "Times New Roman"
    return para
...
def create_methodical_doc():
    ...
    add_paragraph(doc, "Основні характеристики мережі Хопфілда:")
    ...
    add_code_block(doc, "W = (1/N) * Σ(k=1 to P) x_k @ x_k^T")
    ...
    add_heading(doc, "1.2. Асинхронне оновлення", level=2)
    
    add_paragraph(doc, "Асинхронне оновлення станів нейронів:")
    add_code_block(doc, "s_i(t+1) = sign( Σ(j=1 to N) W_ij * s_j(t) )")
    
    add_paragraph(doc, "де sign(x) = +1, якщо x ≥ 0, і -1, якщо x < 0.")
    ...
    add_list_item(doc, "Дослідити роботу з «чужими» літерами")
    ...
    print(f"Методичні вказівки створено: {output_path.absolute()}")


if __name__ == "__main__":
    create_methodical_doc()
